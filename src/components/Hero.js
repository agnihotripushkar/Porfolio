    ...
    return (
        <section id="home" className="relative pt-40 pb-20 px-6 lg:px-8 max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-center min-h-[90vh]">
            <div className="md:w-3/5 text-center md:text-left z-10">
                ...
                >
                    <a href="/Pushkar_Agnihotri_Resume.pdf" target="_blank" rel="noopener noreferrer" onClick={() => trigger('selection')} className="relative group flex items-center gap-2 px-6 py-3 rounded-full text-white font-semibold shadow-[0_0_40px_-10px_rgba(59,130,246,0.5)] transition-all overflow-hidden border border-white/10">
                    ...
                    <a href="https://github.com/agnihotripushkar" target="_blank" rel="noopener noreferrer" onClick={() => trigger('selection')} className="flex items-center gap-2 px-6 py-3 rounded-full bg-white/70 dark:bg-white/5 backdrop-blur-md hover:bg-white dark:hover:bg-white/10 border border-slate-200/50 dark:border-white/10 text-slate-900 dark:text-white text-sm font-semibold transition-all shadow-sm">
                        <FaGithub size={16} /> GitHub
                    </a>

                    <div className="flex gap-2 ml-2">
                        <a href="https://www.linkedin.com/in/pushkaragnihotri/" target="_blank" rel="noopener noreferrer" onClick={() => trigger('selection')} className="flex flex-col justify-center p-2.5 rounded-full bg-white/50 dark:bg-white/5 backdrop-blur-md hover:bg-blue-50 dark:hover:bg-blue-500/20 text-slate-600 hover:text-[#0077B5] border border-slate-200/50 dark:border-white/5 dark:text-slate-300 dark:hover:text-blue-400 transition-all shadow-sm">
                            <FaLinkedin size={20} />
                        </a>
                        <a href="mailto:agnihotripush@gmail.com" onClick={() => trigger('selection')} className="flex flex-col justify-center p-2.5 rounded-full bg-white/50 dark:bg-white/5 backdrop-blur-md hover:bg-red-50 dark:hover:bg-red-500/20 text-slate-600 hover:text-red-500 border border-slate-200/50 dark:border-white/5 dark:text-slate-300 dark:hover:text-red-400 transition-all shadow-sm">
                            <FaEnvelope size={20} />
                        </a>
                        <a href="https://linktr.ee/push1413" target="_blank" rel="noopener noreferrer" onClick={() => trigger('selection')} className="flex flex-col justify-center p-2.5 rounded-full bg-white/50 dark:bg-white/5 backdrop-blur-md hover:bg-emerald-50 dark:hover:bg-emerald-500/20 text-slate-600 hover:text-emerald-500 border border-slate-200/50 dark:border-white/5 dark:text-slate-300 dark:hover:text-emerald-400 transition-all shadow-sm">
                            <FaLink size={20} />
                        </a>
                    </div>
                </motion.div>
